...
//    private int mLastY;
    ...
    public void setView(Context context, View childView) {
        if (childView instanceof ListView) {
            ...
            Log.d("SwipeRefreshView", "获取到listview");
            ...
            Log.d("SwipeRefreshView", "获取到recyclerView");
        }
...
//            Log.d("SwipeRefreshView","快去自定义recyclerView 加footview吧");
//            return;
//        }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.d("SwipeRefreshView", "按下");
                break;
            case MotionEvent.ACTION_MOVE:
                // 移动
                mLastY = (int) event.getRawY();
                final float deltaY = event.getRawY() - mLastY;
                Log.d("SwipeRefreshView", "移动");
...
//                    Log.d("SwipeRefreshView", "能加载...");
//                    loadData();
//                } else {
//                    Log.d("SwipeRefreshView", "不能加载...");
//                }
                break;
        }
...
//                mFooterView.setVisibility(View.VISIBLE);
                if (canLoad(moreData)) {
                    Log.d("SwipeRefreshView", "滚动到最后一行,加载数据，显示footview");
                    loadData();
                }
            }
            Log.d("SwipeRefreshView", "滚动到最后一行");
        } else {
            Log.d("SwipeRefreshView", "没有滚动到最后一行");
            isLastRow = false;
        }
    }
